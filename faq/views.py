...
def article(request, id):
    ...
    article = sorted(Article.objects.filter(categories=name).order_by("-time"), key=lambda np: np.istop(id),
                     reverse=True)
    ...
    if cid == int(id):
        # top 右侧头部文章
        top_new = sorted(Article.objects.filter(categories__id=cat[0].id).order_by("-time"),
                         key=lambda np: np.istop(cat[0].id), reverse=True)  # 右侧文章列表
        top = cat[0]  # 右侧文章标题
        # mid 右侧中部文件
        mid_new = sorted(Article.objects.filter(categories__id=cat[1].id).order_by("-time"),
                         key=lambda np: np.istop(cat[1].id), reverse=True)  # 右侧文章列表
        mid = cat[1]  # 右侧文章标题
    else:
        # top 右侧头部文章
        top_new = sorted(Article.objects.filter(categories__id=ex_cat[0].id).order_by("-time"),
                         key=lambda np: np.istop(ex_cat[0].id), reverse=True)  # 右侧文章列表
        top = ex_cat[0]  # 右侧文章标题
        # mid 右侧中部文件
        mid_new = sorted(Article.objects.filter(categories__id=ex_cat[1].id).order_by("-time"),
                         key=lambda np: np.istop(ex_cat[1].id), reverse=True)  # 右侧文章列表
        mid = ex_cat[1]  # 右侧文章标题
...
def article_show(request, cid, id):
    ...
    top = sorted(Article.objects.filter(categories__id=ex_cat[0].id).order_by("-time"),
                 key=lambda np: np.istop(ex_cat[0].id), reverse=True)

    mid_title = ex_cat[1]  # 右侧中部文章栏目
    mid = sorted(Article.objects.filter(categories__id=ex_cat[1].id).order_by("-time"),
                 key=lambda np: np.istop(ex_cat[1].id), reverse=True)
